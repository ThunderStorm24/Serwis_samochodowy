    ...
    <div class="display-flex flexcolumn align-center  p-20">
        <div class="blok mbottom-20 p-20 w-50">
            <form class="fs-25" method="GET" action="Zarejestruj">
                ...
                <div><input class="formularz w-100" type="text" maxlength="20" name="imie" pattern="[A-Z]{1}[a-z]{2,20}" value="@isset($Imie){{$Imie}}@endif"></input></div>
                <div class="">Nazwisko</div>
                <div class="fs-15">-Rozpoczynać się ma dużą literą</div>
                <div><input class="formularz w-100" type="text" maxlength="30" name="nazwisko" pattern="[A-Z]{1}[a-z]{2,30}" value="@isset($Nazwisko){{$Nazwisko}}@endif"></input></div>
                ...
                <div class="content-space-between display-flex"><input class="formularz w-70" type="text" maxlength="30" name="ulica" pattern="[A-Z]{1}[A-Za-z\s]{2,30}" value="@isset($Ulica){{$Ulica}}@endif"></input>
                    <input class="formularz w-20" type="text" maxlength="5" value="@isset($Nr){{$Nr}}@else __/__ @endif" pattern="^[0-9]+\/[0-9]+" name="nrdom"></input>
                </div>
                <div class="content-space-between display-flex">
                ...
                <div class="content-space-between display-flex"><input class="formularz w-70" type="text" maxlength="30" name="miasto" pattern="[A-Z]{1}[A-Za-z\s]{2,30}" value="@isset($Miasto){{$Miasto}}@endif"></input>
                    <input class="formularz w-20" type="text" maxlength="6" value="@isset($Kod){{$Kod}}@else __-___ @endif" pattern="^[0-9]{2}\-[0-9]{3}" name="kod"></input>
                </div>
                <div>Numer Telefonu</div>
                <div class="fs-15">w postaci <b>+48</b>123456789</div>
                <div><input class="formularz w-100" type="tel" maxlength="12" value="@isset($Telefon){{$Telefon}}@else +48 @endif" pattern="^[0-9\-\+]{12,12}$" required name="telefon"></input></div>
                <div>Mail</div>
                <div class="fs-15">w postaci tom<b>@</b>onet.pl</div>
                <div><input class="formularz w-100" type="email" maxlength="40" pattern="^[a-z0-9]+\@[a-z]+\.[a-z]+" name="mail" value="@isset($Mail){{$Mail}}@endif"></input></div>
                <div>Login</div>
                <div class="fs-15">-Musi posiadać od 3-40 znaków</div>
                <div><input class="formularz w-100" type="text" maxlength="40" name="login" pattern=".\S{2,40}" value="@isset($Login){{$Login}}@endif"></input></div>
                <div>Hasło</div>
                <div class="fs-15">-Musi posiadać od 8-40 znaków</div>
                <div class="fs-15">-Musi się zaczynać od dużej litery</div>
                <div><input class="formularz w-100" type="text" maxlength="40" name="haslo" pattern="^[A-Z]{1}.\S{6,40}" value="@isset($Haslo){{$Haslo}}@endif"></input></div>
                <div>Potwierdź Hasło</div>
                <div class="fs-15">-Hasła muszą się zgadzać!</div>
                <div><input class="formularz w-100" type="text" maxlength="40" name="phaslo" pattern="^[A-Z]{1}.\S{6,40}" value="@isset($pHaslo){{$pHaslo}}@endif"></input></div>
                @isset($messageHaslo)<div class="fs-15"><b>{{$messageHaslo}}</b></div>@endif
